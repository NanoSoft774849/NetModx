    ...
    inout ingress_intrinsic_metadata_for_tm_t        ig_tm_md)
    {
       ...
       Forwarder() nexthop;
        ...
        action initialize()
        {
            ...
            // we don't need to initialize this becuase in each modulation type is already selected by the payload selector table.
            // hdr.circ.current_payload_b = hdr.payload_bpsk.b0;
       ...
       apply{
           ...
           if(hdr.mod.isValid() && !done)
           {
            ...
            // Modulation....
               if(hdr.mod.bps == 1)
               {
                   hdr.tmp_iq_bpsk.setValid();
                   BPSK.apply(hdr, meta);
                   enable_circ = true;
               }
               else if( hdr.mod.bps == 2)
               {
                    hdr.tmp_iq_qpsk.setValid();
                    QPSK.apply(hdr, meta);
                    enable_circ = true;
               }
               else if(hdr.mod.bps == 4)
               {
                    hdr.tmp_iq_qam16.setValid();
                    QAM16.apply(hdr, meta);
                    enable_circ = true;
               }
               else if(hdr.mod.bps == 6)
               {
                    hdr.tmp_iq_qam64.setValid();
           ...
           {
               hdr.circ.setInvalid();
               hdr.tmp_iq_bpsk.setInvalid(); // reset tmp//
               hdr.tmp_iq_qpsk.setInvalid(); // reset tmp//
               hdr.tmp_iq_qam16.setInvalid(); // reset tmp//
               hdr.tmp_iq_qam64.setInvalid(); // reset tmp//
               hdr.tmp_iq_qam256.setInvalid(); // reset tmp//
