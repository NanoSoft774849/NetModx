...
#define GEN_ACTION(i)\
        action net8##i(){\
        hdr.tmp_iq_qam256.s##i = (mc_iq_qam256_t) act8.execute((bit<32>)hdr.circ.current_payload_b[7:0]);\
...
//
    ...
    table QAM256_table 
    {
     ...
     NET_MOD_PAYLOAD_SELECTOR(qam256)

     apply{
        
        if( hdr.circ.payload_b_shift_cntr >= PB_QAM256_BITS_PER_B || hdr.circ.payload_b_shift_cntr == 0)
        {
           
           hdr.circ.payload_b_shift_cntr = 0;
           payload_selector_tbl_qam256.apply();
