...
#define GEN_ACTION(i)\
        action net##i(){\
        hdr.tmp_iq_bpsk.s##i = (mc_iq_bpsk_t) act1.execute((bit<32>)hdr.circ.current_payload_b[0:0]);\
...
//
    ...
    apply{
        ...
        if( hdr.circ.payload_b_shift_cntr >= PB_BPSK_BITS_PER_B || hdr.circ.payload_b_shift_cntr == 0)
        {
           
           hdr.circ.payload_b_shift_cntr = 0;
           payload_selector_tbl_bpsk.apply();
