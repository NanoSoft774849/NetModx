...
#define GEN_ACTION(i)\
        action net##i(){\
        hdr.tmp_iq_qpsk.s##i = (mc_iq_qpsk_t) act2.execute((bit<32>)hdr.circ.current_payload_b[1:0]);\
...
//
    ...
    apply{
        ...
        if( hdr.circ.payload_b_shift_cntr >= PB_QPSK_BITS_PER_B || hdr.circ.payload_b_shift_cntr == 0)
        {
           
           hdr.circ.payload_b_shift_cntr = 0;
           payload_selector_tbl_qpsk.apply();
